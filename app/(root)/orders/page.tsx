...
const OrdersPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {!loading && (safeOrders.length === 0) && <div className="text-black">No orders found.</div>}
      ...
      <div className="space-y-4">
        {safeOrders.map((o) => {
          ...
          return (
          <Link key={o.id} href={`/orders/${o.id}`} className="block border border-gray-200 rounded p-4 hover:shadow">
            <div className="flex justify-between">
              <div className="text-black font-semibold">Order #{idShort}</div>
              <div className="text-gray-700">{o.status} / {o.paymentStatus}</div>
            </div>
            <div className="text-black mt-2">Total: ${totalVal.toFixed(2)}</div>
